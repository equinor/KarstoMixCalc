...
namespace MixCalc
{
    class OpcClient : IDisposable
    {
        ...
        public OpcClient(string endpointUrl, string username, string password)
        {
        ...
        public async Task Connect()
        {
            try
            {
                ...
                OpcSession = await Session.Create(config, endpoint, false, "OPC UA Console Client", 60000, user, null).ConfigureAwait(false);
